...
export default function ClientDashboardContent({ data, formatDate, actionLoading, handleAccept, handlePay, handleReject, handleHire, setWorkViewModal, setReviewModal, setWorkspaceModal, setManageGigModal }) {
  ...
  useEffect(() => {
    ...
    if (viewParam === 'billing' || tabParam === 'invoices') {
      setActiveView('billing');
    } else {
      setActiveView('lifecycle');
      if (tabParam && ['posted-jobs', 'applicants', 'active-projects', 'submitted-work', 'payments', 'completed-projects'].includes(tabParam)) {
  ...
  // 6 Lifecycle Stages Tabs List (Lifecycle ONLY)
  const tabs = [
  ...
  return (
    <div className="space-y-10">
      ...
      <div className="flex flex-col sm:flex-row items-center justify-between gap-4 border-b border-gray-100 pb-6">
        <div className="flex items-center gap-2 bg-gray-100 p-1.5 rounded-2xl w-full sm:w-auto">
          <button
            ...
            className={`flex-1 sm:flex-none px-6 py-3 rounded-xl text-xs font-black uppercase tracking-widest transition-all duration-200 flex items-center justify-center gap-2 ${
              activeView === 'lifecycle'
                ? 'bg-black text-white shadow-sm'
                : 'text-gray-500 hover:text-gray-900 hover:bg-white/50'
            }`}
          >
            <Briefcase className="w-4 h-4" /> Gig Workspace
          </button>
          <button
          ...
          >
            <CreditCard className="w-4 h-4" /> Billing & Invoices Hub
            <span className={`text-[10px] font-extrabold px-2 py-0.5 rounded-md ${
              activeView === 'billing' ? 'bg-white/20 text-white' : 'bg-gray-200 text-gray-600'
            }`}>
              {invoices.length}
            </span>
          </button>
      ...
      {/* VIEW 1: GIG WORKSPACE & LIFECYCLE MANAGEMENT */}
      {activeView === 'lifecycle' && (
      ...
      {/* VIEW 2: STANDALONE BILLING & INVOICES HUB */}
      {activeView === 'billing' && (() => {
